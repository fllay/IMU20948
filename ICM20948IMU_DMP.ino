...
#define ACCEL_FACTOR                      2.0/16384.0*9.80665   // (ADC_Value / Scale) * 9.80665            => Range : +- 2[g]   //  Scale : +- 16384
...
void loop()
{
  ...
  if ((myICM.status == ICM_20948_Stat_Ok) || (myICM.status == ICM_20948_Stat_FIFOMoreDataAvail)) // Was valid data available?
  {
    ...
    if ((data.header & DMP_header_bitmap_Quat6) > 0) // Check for orientation data (Quat9)
    {
      ...
      double yaw = atan2(siny_cosp, cosy_cosp);
       ...
       SERIAL_PORT.print("Orientation: ");
        ...
        SERIAL_PORT.print(yaw*180/M_PI);

        SERIAL_PORT.print(" ");

        SERIAL_PORT.print(pitch*180/M_PI);

        SERIAL_PORT.print(" ");

        SERIAL_PORT.println(roll*180/M_PI);*/
      
      }

    if ((data.header & DMP_header_bitmap_Accel) > 0) // Check for Accel
    {
      float acc_x = (float)data.Raw_Accel.Data.X*ACCEL_FACTOR; // Extract the raw accelerometer data
      float acc_y = (float)data.Raw_Accel.Data.Y*ACCEL_FACTOR;
      float acc_z = (float)data.Raw_Accel.Data.Z*ACCEL_FACTOR;
    ...
    {
      float x = (float)data.Raw_Gyro.Data.X*GYRO_FACTOR; // Extract the raw gyro data
      float y = (float)data.Raw_Gyro.Data.Y*GYRO_FACTOR;
      float z = (float)data.Raw_Gyro.Data.Z*GYRO_FACTOR;
    ...
    {
      float x = (float)data.Compass.Data.X*MAG_FACTOR; // Extract the compass data
      float y = (float)data.Compass.Data.Y*MAG_FACTOR;
      float z = (float)data.Compass.Data.Z*MAG_FACTOR;
